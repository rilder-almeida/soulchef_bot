...
from constants import pagseguro_token, soulchef_email
...
class PagSeguroTransaction(PagSeguroBase):
    URLS = {
        ...
        'request_redirect': 'https://pagseguro.uol.com.br/v2/checkout/payment.html?code={code}',  # noqa
    }
    ...
    def get_checkout_url(self):
        response = requests.post(
            self.URLS['request'], data=self.get_dados(), headers=self.headers
        )
    ...
    def __dict__(self):
        ...
        if self.item:
            ...
            data['itemAmount1'] = ("{0:.2f}".format(
                round(self.item['amount'], 2)),)
        ...
        if self.discount:
            data['extraAmount'] = ("{0:.2f}".format(
                round(self.discount, 2)),)
        return data
...
class PagSeguroSignature(PagSeguroTransaction):
    URLS = {
        ...
        'request_redirect': 'https://pagseguro.uol.com.br/v2/pre-approvals/request.html?code={code}',  # noqa
    }
    ...
    def __init__(
        self, token, email, name, description, price,
        reference=None, period='MONTHLY'
    ):
        if len(name) > 100:
    ...
    def __dict__(self):
        data = {
            ...
            'preApprovalAmountPerPayment': "{0:.2f}".format(
                round(self.price, 2)),
            'preApprovalPeriod': self.period,
            'preApprovalMaxPaymentsPerPeriod': 1,
            'preApprovalMaxAmountPerPeriod': "{0:.2f}".format(
                round(self.price, 2)),
            'preApprovalMaxTotalAmount': "{0:.2f}".format(
                round(self.price * 12, 2)),
...
class PagSeguroSender(object):
    def __init__(self, name=None, ddd=None, phone=None,
                 email=None, city=None, uf=None):
...
class PagSeguroNotificationHandler(PagSeguroBase):
    URLS = {
        'check_notification': 'https://ws.pagseguro.uol.com.br/v2/transactions/notifications/{code}?email={email}&token={token}',  # noqa
    }
    ...
    def get_check_notification_url(self):
        return self.URLS['check_notification'].format(
            code=self.code, email=self.email, token=self.token
        )
...
class PagSeguroNotificationSignatureHandler(PagSeguroNotificationHandler):
    URLS = {
        'check_notification': 'https://ws.pagseguro.uol.com.br/v2/pre-approvals/notifications/{code}?email={email}&token={token}',  # noqa
        'check_assinatura': 'https://ws.pagseguro.uol.com.br/v2/pre-approvals/{code}?email={email}&token={token}',  # noqa
...
class PagSeguroSignatureResponse(PagSeguroAbstractResponse):
    def __init__(
        self, code, date, tracker, status, reference,
        lasteventdate, charge, soup_sender
    ):
        self.code = code
...
class PagSeguroNotificationResponse(PagSeguroAbstractResponse):
    ...
    TRANSACTION_TYPE = {1: 'Pagamento', 11: 'Assinatura'}

    def __init__(self, code, reference, transactiontype,
                 status, amount, soup_sender):
        self.code = code
        self.reference = reference
...
class PagSeguroSignatureCharger(PagSeguroTransaction):
    ...
    def __init__(self, token, email, code, items=[], reference=None):
        super(PagSeguroSignatureCharger, self).__init__(
    ...
    def get_data(self):
        data = {'email': self.email, 'token': self.token,
    ...
    def get_items_data(self):
        ...
        for index, item in enumerate(self.items):
            ...
            data['itemDescription{index}'.format(
                index=index + 1)] = item['description']
            data['itemAmount{index}'.format(
                index=index + 1)] = "{:.2f}".format(
                item['amount']
            )
            data['itemQuantity{index}'.format(
                index=index + 1)] = item['quantity']
        return data

    def charge(self):
        response = requests.post(
            self.URLS['payment'], data=self.get_data(), headers=self.headers
        )
        if response.status_code == 401:
            raise UnauthorizedException(u'invalid Token ou E-mail')
...
transaction = PagSeguroTransaction(
    pagseguro_token, soulchef_email, reference='Referencia de seu sistema'
)
